...
def train():
    ...
    '''# print('unlabeled_sampler: ', unlabeled_sampler)
    unlabeled_loader_weak = torch.utils.data.DataLoader(data_weak, batch_size=batch_size,
                                                   sampler=unlabeled_sampler, num_workers=num_workers,
                                                   collate_fn=detection_collate, pin_memory=True)
    # print('unlabeled_sampler: ', unlabeled_sampler)
    unlabeled_loader_strong = torch.utils.data.DataLoader(data_strong, batch_size=batch_size,
                                                   sampler=unlabeled_sampler, num_workers=num_workers,
                                                   collate_fn=detection_collate, pin_memory=True)

    # print('labeled_sampler: ', labeled_sampler)
    labeled_loader_weak = torch.utils.data.DataLoader(data_weak, batch_size=batch_size,
                                                 sampler=labeled_sampler, num_workers=num_workers,
                                                 collate_fn=detection_collate, pin_memory=True)
    # print('labeled_sampler: ', labeled_sampler)
    labeled_loader_strong = torch.utils.data.DataLoader(data_strong, batch_size=batch_size,
                                                 sampler=labeled_sampler, num_workers=num_workers,
                                                 collate_fn=detection_collate, pin_memory=True)'''
    ...
    for iteration in range(start_iter, max_iter):
        ...
        # semi-supervised
            ...
            # Load labeled data
            if (iteration - burn_in_iter) % epoch_size_labeled == 0:
                batch_iterator_label = data_prefetcher2(labeled_loader, device)
            ...
            '''
            # learning rate decay
            if epoch < (epoch_max*0.65):
                lr = 0.001
                for param_group in optimizer.param_groups:
                    param_group['lr'] = lr
            elif (epoch_max*0.65) <= epoch < (epoch_max*0.8):
                lr = 0.0001
                for param_group in optimizer.param_groups:
                    param_group['lr'] = lr
            elif (epoch_max*0.8) <= epoch < (epoch_max*0.875):
                lr = 0.00001
                for param_group in optimizer.param_groups:
                    param_group['lr'] = lr
            elif (epoch_max * 0.875) <= epoch < (epoch_max * 0.975):
                lr = 0.000001
                for param_group in optimizer.param_groups:
                    param_group['lr'] = lr
            else:
