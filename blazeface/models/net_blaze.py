...
class Blaze(nn.Module):
    ...
    def forward(self,inputs):
        ...
        for (x, l, c) in zip(detections, self.loc, self.conf):
            loc.append(l(x).permute(0, 2, 3, 1).contiguous())
            conf.append(c(x).permute(0, 2, 3, 1).contiguous())
            # landm.append(lam(x).permute(0, 2, 3, 1).contiguous())
            # print('l(x).permute(0, 2, 3, 1).contiguous(): ', l(x).permute(0, 2, 3, 1).contiguous().shape)
            # print('c(x).permute(0, 2, 3, 1).contiguous(): ', c(x).permute(0, 2, 3, 1).contiguous().shape)
